...
function uploadFileToS3WithProgress(
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      xhr.onerror = () =>
        reject(new Error("S3 upload network error (CORS or network)"));
      xhr.send(file);
    } catch (err) {
      reject(err);
    }
...
export async function fetchQuestions(
  ...
  if (params.professorId)
    searchParams.append("professorId", params.professorId.toString());
  if (params.keyword) searchParams.append("keyword", params.keyword);
  if (params.status) searchParams.append("status", params.status);

  const queryString = searchParams.toString();
  const url = queryString
    ? `${BASE_URL}/api/questions?${queryString}`
...
export async function createQuestion(
  ...
  opts?: {
    onProgress?: (info: { index: number; file: File; percent: number }) => void;
  }
  ...
  if (questionData.files && questionData.files.length > 0) {
    imageUrls = await presignAndUpload(
      "uploads/questions",
      questionData.files,
      opts?.onProgress
    );
...
export async function fetchCombinedAnswers(
  ...
  const params = new URLSearchParams({
    questionPostId: String(questionPostId),
    page: String(page),
    size: String(size),
  });
  const response = await authenticatedFetch(
    `${BASE_URL}/api/answers/combined?${params.toString()}`,
    { method: "GET" }
  );
  if (!response.ok) {
...
export async function deleteAnswerById(id: number): Promise<void> {
  const response = await authenticatedFetch(`${BASE_URL}/api/answers/${id}`, {
    method: "DELETE",
  });
  if (!response.ok) {
    const t = await response.text().catch(() => "");
...
export async function createAnswer(
  ...
  const payload: {
    ...
    questionPostId: Number(
      (answerData as { questionId?: string; questionPostId?: number })
        .questionId ??
        (answerData as { questionPostId?: number }).questionPostId ??
        0
    ),
    ...(imageUrls ? { imageUrls } : {}),
  };
...
export async function adoptAnswer(answerPostId: number): Promise<void> {
  const qs = new URLSearchParams({
    answerPostId: String(answerPostId),
  }).toString();
  const response = await authenticatedFetch(
    `${BASE_URL}/api/answers/selection?${qs}`,
    { method: "POST" }
  );
  if (!response.ok) {
    let msg = "";
    ...
    throw new Error(
      `Failed to adopt answer: ${response.status} ${msg || ""}`.trim()
    );
  }
}
...
export async function unadoptAnswer(questionPostId: number): Promise<void> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to unadopt answer: ${response.status} ${msg || ""}`.trim()
    );
  }
}
...
export async function fetchUserProfile(): Promise<User> {
  ...
  if (!response.ok) {
    console.log(
      `사용자 프로필 조회 실패: ${response.status} ${response.statusText}`
    );
...
export async function createComment(
  ...
  const payload: {
    ...
    answerPostId: data.answerPostId,
...
export async function fetchUserPosts(
  ...
  if (process.env.NODE_ENV === "development")
    console.log("GET /api/users/posts:", url);
  const res = await authenticatedFetch(url, { method: "GET" });
  if (!res.ok) {
    const t = await res.text().catch(() => "");
...
export async function createAiProblem(payload: {
  subjectId: number;
  file: File;
}): Promise<Blob> {
