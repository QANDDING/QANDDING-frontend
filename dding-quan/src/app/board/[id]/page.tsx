...
type Combined = {
  ...
  users: {
    content: Array<{ id: string; title?: string; content: string; isAdopted?: boolean; createdAt?: string; imageUrls?: string[]; authorNickname?: string; nickname?: string }>;
    number?: number;
    size?: number;
...
export default function BoardDetailPage({ params }: { params: Promise<{ id: string }> }) {
      ...
      if (page > 0 && comments[key]) {
        ...
        setComments((prev) => ({
          ...
          [key]: {
            ...(prev[key] || {}),
            ...res,
            content: [...(prev[key]?.content || []), ...(res.content || [])],
          } as CommentPage,
        }));
      } else {
        // replace
  ...
  async function removeComment(commentId: number, answerId: number, imageCount?: number) {
    ...
    const warn = c > 0 ? `이 댓글에는 이미지 ${c}장이 첨부되어 있습니다. 삭제 후 복구할 수 없습니다.\n정말 삭제할까요?` : '댓글을 삭제하시겠습니까? 삭제 후 복구할 수 없습니다.';
  ...
  function CommentImages({ urls }: { urls: string[] }) {
    ...
    return (
      <div className='mt-2 inline-block'>
        <button type='button' onClick={() => setImageModal({ urls, index: 0 })} className='relative block rounded border overflow-hidden'>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={cover} alt={`첨부 이미지 대표`} className='w-28 h-20 object-cover' />
          <span className='absolute bottom-1 right-1 px-1.5 py-0.5 text-[10px] rounded bg-black/70 text-white'>{count}장</span>
        </button>
      </div>
    );
  }
  ...
  async function loadAll() {
    setLoading(true);
    try {
      const [q, a] = await Promise.all([questionApi.getById(id), answerApi.getCombined(Number(id), 0, 3)]);
      ...
      const authorNick = (raw['authorNickname'] as string) ?? (authorObj?.['nickname'] as string) ?? (writerObj?.['nickname'] as string) ?? (ownerObj?.['nickname'] as string);
      setQuestion({
        ...(raw || {}),
        id: String(raw['id']),
      ...
      setAnswers((prev) => {
        ...
        const merged = {
          ...
          users: {
            ...(prev.users || {}),
            ...moreUsers,
            content: [...((prev.users && prev.users.content) || []), ...(moreUsers.content || [])],
          },
        } as Combined;
        return merged;
      });
  ...
  const getQuestionAuthorId = (q: Question | null): string | null => {
    ...
    const cand = [anyQ['authorId'], anyQ['userId'], anyQ['memberId'], anyQ['writerId'], anyQ['ownerId'], getNestedId(anyQ['author']), getNestedId(anyQ['writer']), getNestedId(anyQ['owner'])];
    for (const v of cand) {
  ...
  const getQuestionAuthorNickname = (q: Question | null): string | null => {
    ...
    const cand = [(q as Question).authorNickname, getNestedNickname(anyQ['author']), getNestedNickname(anyQ['writer']), getNestedNickname(anyQ['owner'])];
    for (const v of cand) {
  ...
  async function submitAnswer() {
    ...
    try {
      ...
      await answerApi.create(
        { content, questionId: id, title, files: answerFiles },
        {
          onProgress: ({ index, percent }) => {
  ...
  return (
    <main className='mx-auto max-w-3xl px-6 py-10'>
      <div className='flex items-center justify-between mb-4 gap-3'>
        ...
        <div className='flex items-center gap-2'>
          {!isAuthor && (
            <button type='button' onClick={() => setAnswering((v) => !v)} className='px-3 py-1.5 rounded-md text-sm bg-emerald-600 text-white hover:bg-emerald-700'>
              답변달기
            </button>
          )}
          {isAuthor && (
            <>
              <Link
                href={`/ask?edit=true&id=${question?.id}&title=${encodeURIComponent(question?.title || '')}&content=${encodeURIComponent(question?.content || '')}`}
                className='px-3 py-1.5 rounded-md text-sm bg-blue-600 text-white hover:bg-blue-700'>
                질문 수정
              </Link>
              <button type='button' onClick={deleteQuestionPost} disabled={deletingQuestion} className='px-3 py-1.5 rounded-md text-sm bg-red-600 text-white hover:bg-red-700 disabled:opacity-50'>
      ...
      <div className='text-xs text-gray-500 mb-2 flex items-center justify-between gap-2 flex-wrap'>
        <div className='flex items-center gap-2 flex-wrap'>
          ...
          {professorLabel && <span className='px-2 py-0.5 bg-gray-100 rounded-full'>교수: {professorLabel}</span>}
        </div>
        <div className='flex items-center gap-2'>
          {question?.authorNickname && <span>작성자: {question.authorNickname}</span>}
          {question?.createdAt && <span>{formatKST(question.createdAt)}</span>}
        </div>
      </div>
      <div className='rounded-md bg-gray-50 p-4 mb-2'>
        <div className='text-sm font-medium text-gray-900 mb-2'>본문</div>
        <div className='text-sm text-gray-700 whitespace-pre-line'>{question?.content || (loading ? '내용을 불러오는 중입니다.' : '-')}</div>
        {(() => {
          const urls = question?.imageUrls && question.imageUrls.length > 0 ? question.imageUrls : (question?.files || []).map((f) => f.url).filter(Boolean);
        ...
        <section className='mb-6'>
          ...
          <div className='rounded-md border p-4 bg-white'>
            <div className='flex items-center justify-between mb-1'>
              <div className='font-semibold text-gray-900 truncate'>A. {answers.ai?.title || '답변'}</div>
              <div className='flex items-center gap-2 text-xs text-gray-500'>
                <span className='px-2 py-0.5 bg-blue-50 text-blue-700 rounded-full'>AI</span>
                {answers.ai?.createdAt && <span>{formatKST(answers.ai.createdAt)}</span>}
              </div>
            </div>
            <div className='text-sm font-medium text-gray-900 mb-1'>본문</div>
            <CollapsibleText text={answers.ai.content} />
            {answers.ai?.imageUrls && answers.ai.imageUrls.length > 0 && <ImageCarousel urls={answers.ai.imageUrls} />}
          </div>
        </section>
      )}
            ...
            <ul className='space-y-3 mb-4'>
              {list.map((u: UserAnswer) => (
                <li key={u.id} id={`answer-${u.id}`} className='rounded-md border p-4 bg-white'>
                  <div className='flex items-start justify-between mb-1 gap-2'>
                    ...
                    <div className='flex items-center gap-2'>
                      ...
                      {!Boolean(u.isAdopted ?? u.adopted) && isAuthor && !anyAdopted && (
                        <button onClick={() => adopt(Number(u.id))} disabled={adopting === Number(u.id)} className='px-2.5 py-1 rounded-md bg-emerald-600 text-white text-xs disabled:opacity-50'>
                  ...
                  {u.imageUrls && u.imageUrls.length > 0 && <ImageCarousel urls={u.imageUrls} />}
                  {/* 댓글 토글 및 영역 */}
                  <div className='mt-3'>
                    <button type='button' onClick={() => toggleComments(Number(u.id))} className='inline-flex items-center gap-1 text-xs text-gray-700 bg-gray-100 hover:bg-gray-200 px-2 py-1 rounded'>
                      <span>{openComments[String(u.id)] ? '댓글 숨기기' : `댓글 보기 (${commentTotals[String(u.id)] ?? calcCommentCount(Number(u.id))})`}</span>
                    ...
                    {openComments[String(u.id)] && (
                      <div className='mt-2 border-t pt-3'>
                        ...
                        {/* 댓글 목록 */}
                        {loadingComments[String(u.id)] && <div className='text-xs text-gray-500'>불러오는 중…</div>}
                        {!loadingComments[String(u.id)] && (
                          <>
                            <ul className='space-y-3'>
                                  ...
                                  <div className='p-2 rounded-md bg-gray-50 border'>
                                    <div className='flex items-center justify-between text-xs text-gray-600'>
                                      <span className='font-medium inline-flex items-center gap-1'>
                                        ...
                                        {isQuestionAuthorComment(th.parent?.nickname) && <span className='px-1.5 py-0.5 rounded-full bg-green-100 text-green-700 text-[10px]'>작성자</span>}
                                      </span>
                                    ...
                                    <div className='mt-1 flex flex-col sm:flex-row sm:items-start sm:gap-3'>
                                      <div className='text-gray-800 whitespace-pre-line flex-1'>{th.parent?.content}</div>
                                          ...
                                          <li key={r.id} className='p-2 rounded-md bg-white border'>
                                            <div className='flex items-center justify-between text-xs text-gray-600'>
                                              <span className='font-medium inline-flex items-center gap-1'>
                                                ...
                                                {isQuestionAuthorComment(r.nickname) && <span className='px-1.5 py-0.5 rounded-full bg-green-100 text-green-700 text-[10px]'>작성자</span>}
                                              </span>
                                              {r.createdAt && <span>{formatKST(r.createdAt)}</span>}
                                            </div>
                                            <div className='mt-1 flex flex-col sm:flex-row sm:items-start sm:gap-3'>
                                              <div className='text-gray-800 whitespace-pre-line flex-1'>{r.content}</div>
                                          ...
                                          <div className='mt-1'>
                                            <input
                                              ...
                                              onKeyDown={(e) => {
                                                if (e.key === 'Enter') {
                                                  e.preventDefault();
                                                  submitReply(Number(th.parent?.id), Number(u.id));
                                                }
                                              }}
                              ...
                              {(comments[String(u.id)]?.content || []).length === 0 && <li className='text-xs text-gray-500'>댓글이 없습니다.</li>}
                            </ul>
              ...
              {(!answers || (answers.users?.content || []).length === 0) && !loading && <li className='text-sm text-gray-500'>아직 등록된 사용자 답변이 없습니다.</li>}
            </ul>
          );
        })()}
        {userHasMore && (
          <div className='mb-6'>
            <button type='button' onClick={loadMoreUsers} disabled={loadingMore} className='px-3 py-1.5 rounded-md border text-sm bg-white hover:bg-gray-50 disabled:opacity-50'>
      ...
      {answering && !isAuthor && (
        <div
          ...
          <div className='w-full max-w-xl bg-white rounded-lg shadow-lg border p-4' onClick={(e) => e.stopPropagation()}>
            ...
            <div className='mt-3 flex items-center justify-end gap-2'>
              ...
              <button type='button' onClick={submitAnswer} disabled={answerSubmitting} className='px-3 py-1.5 rounded-md bg-emerald-600 text-white text-sm disabled:opacity-50'>
      ...
      {imageModal && (
        <div className='fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4' onClick={() => setImageModal(null)}>
          <div className='relative max-w-[90vw] max-h-[85vh]'>
            ...
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={imageModal.urls[imageModal.index]} alt='첨부 이미지 확대' className='max-w-[90vw] max-h-[85vh] rounded shadow-lg' onClick={(e) => e.stopPropagation()} />
